...
use caxton::performance::message_routing::{
    OptimizedMessageRouter, FipaMessage, Performative, MessagePriority, AgentId, RouterConfig,
};
use bytes::Bytes;
use uuid::Uuid;
...
fn bench_agent_operations(c: &mut Criterion) {
    ...
    group.bench_function("register_agent", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            let result = router.register_agent(
                agent_id,
                "localhost:8080".to_string(),
...
fn bench_routing_configurations(c: &mut Criterion) {
    ...
    group.bench_function("small_batches_frequent_flush", |b| {
        b.to_async(&rt).iter(|| async {
            ...
            // Route messages
            for message in &messages[..100] { // Subset for benchmarking
