...
## 4. Send a Message to Your Agent

Send messages to your deployed agent:

```bash
# Send request to DataAnalyzer agent
caxton message send \
  --agent "DataAnalyzer" \
  --content '{
...
The message router delivers your request to the specified agent.
...
├─ REQUEST DataAnalyzer: "Create Q3 sales report"
├─ RESPONSE ReportGenerator: "Analysis complete: {results}"
└─ RESPONSE user: "Report generated: report.pdf"
```
...
- **Agent Map**: Visual representation of available agents
...
Agents communicate via direct messaging with simple routing:

- **Direct routing**: Send messages to specific agents by name
- **Load balancing**: Distribute work across multiple agent instances
- **Easy scaling**: Deploy multiple instances of the same agent
...
Simple message patterns ensure robust agent communication:

- **Direct patterns**: Straightforward agent messaging
...
1. **Use direct agent messaging** for clear communication patterns
