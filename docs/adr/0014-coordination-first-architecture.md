...
ADR-0013 proposed using PostgreSQL for state management with event sourcing
and snapshots. However, after careful analysis, this approach:
...
Further research revealed that Caxton's actual needs are **coordination**
...
Caxton adopts a **coordination-first architecture** that eliminates shared
state in favor of lightweight coordination protocols. Agent state management
becomes a business domain responsibility through MCP tools.

**Protocol Layering**:

- **SWIM Protocol**: Infrastructure layer for cluster coordination and
  membership
- **FIPA Protocol**: Application layer for semantic agent-to-agent
  messaging
- **Clear Separation**: These protocols complement rather than compete
  with each other
...
- **Eventual consistency**: Agent registry may be temporarily
  inconsistent
...
- **MCP tool requirement**: Businesses must provide state tools if
  needed
- **Migration complexity**: Existing systems expecting shared state need
  updates
...
- [ADR-0013: State Management Architecture (Superseded)](
...
- [Lightweight State Alternatives Research](
  ../research/lightweight-state-alternatives.md)
