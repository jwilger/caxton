...
We will use WebAssembly (WASM) as the execution environment for all agents
in Caxton.
...
- **True isolation**: WASM's sandboxing ensures agents cannot interfere
  with each other
...
- **Limited system access**: Agents can only do what the host
  explicitly allows
