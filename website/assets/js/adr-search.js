...
(function () {
  ...
  function initADRSearch() {
    ...
    console.log(
      "ADR Search: Successfully initialized with",
      searchIndex.length,
      "ADRs",
    );
  }
  ...
  function buildSearchIndex() {
    ...
    adrCards.forEach((card) => {
      const titleElement =
        card.querySelector(".adr-title a") || card.querySelector(".adr-title");
      const excerptElement = card.querySelector(".adr-excerpt");
      const statusElement = card.querySelector(".adr-status");
      const linkElement =
        card.querySelector(".adr-title a") || card.querySelector("a");
    ...
    timelineItems.forEach((item) => {
      ...
      if (
        titleElement &&
        !searchIndex.find(
          (indexed) => indexed.title === titleElement.textContent.trim(),
        )
      ) {
        const indexItem = {
          title: titleElement.textContent.trim(),
      ...
      switch (e.key) {
        case "ArrowDown":
          ...
          if (activeResult) {
            ...
            newIndex =
              currentIndex < resultItems.length - 1 ? currentIndex + 1 : 0;
          } else {
            newIndex = 0;
          }
          break;
        ...
        case "ArrowUp":
          ...
          if (activeResult) {
            const currentIndex = Array.from(resultItems).indexOf(activeResult);
            newIndex =
              currentIndex > 0 ? currentIndex - 1 : resultItems.length - 1;
          } else {
  ...
  function filterVisibleContent(query) {
    ...
    adrCards.forEach((card) => {
      const title =
        card.querySelector(".adr-title")?.textContent.toLowerCase() || "";
      const excerpt =
        card.querySelector(".adr-excerpt")?.textContent.toLowerCase() || "";
      const status =
        card.querySelector(".adr-status")?.textContent.toLowerCase() || "";
    ...
    timelineItems.forEach((item) => {
      const title =
        item.querySelector(".timeline-link h5")?.textContent.toLowerCase() ||
        "";
      const status =
        item.querySelector(".status")?.textContent.toLowerCase() || "";
