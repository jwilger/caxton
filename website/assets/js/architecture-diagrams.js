...
class ArchitectureDiagrams {
  constructor() {
    ...
    this.animationEnabled = !window.matchMedia(
  ...
  createWasmIsolationDiagram(container) {
    ...
    this.addTooltips(svg, [
      {
        element: agents[0],
        text: "Each agent runs in an isolated WASM sandbox with restricted system access",
      },
      {
        x: 220,
        y: 175,
        text: "Message Bus handles secure inter-agent communication via agent messaging protocols",
      },
      {
        x: 310,
        y: 175,
        text: "Resource Manager enforces memory and CPU limits per agent",
      },
    ]);
  ...
  createFipaMessageFlowDiagram(container) {
    ...
    flows.forEach((flow, index) => {
      ...
      this.addPath(
        svg,
        `M ${flow.from.x} ${flow.from.y + offsetY} Q ${(flow.from.x + flow.to.x) / 2} ${midY} ${flow.to.x} ${flow.to.y + offsetY}`,
        {
          fill: "none",
          stroke: flow.color,
          strokeWidth: 3,
      ...
      if (this.animationEnabled) {
        ...
        const animCircle = this.addCircle(
          svg,
          flow.from.x,
          flow.from.y + offsetY,
          4,
    ...
    const steps = [
      {
        step: 1,
        text: "Initiator broadcasts\nCall for Proposals (CFP)",
        y: 470,
      },
      { step: 2, text: "Participants evaluate\nand send PROPOSE", y: 510 },
      {
        step: 3,
        text: "Initiator selects best\nproposal and sends ACCEPT",
        y: 550,
      },
      {
        step: 4,
        text: "Rejected participants\nreceive REJECT messages",
        y: 590,
      },
      {
        step: 5,
        text: "Winner completes task\nand sends INFORM result",
        y: 630,
      },
    ];
  ...
  createObservabilityPipelineDiagram(container) {
    ...
    this.addText(
      ...
      500,
      725,
      "Dashboards • Alerts • Query Builder • Data Correlation",
      {
        fontSize: "12px",
        textAnchor: "middle",
        fill: this.colors.subtext0,
  ...
  createMultiLanguageDiagram(container) {
    ...
    this.addText(
      ...
      450,
      135,
      "System Calls • File I/O • Network • Threading • Memory Management",
      {
        fontSize: "12px",
        textAnchor: "middle",
        fill: this.colors.subtext0,
    ...
    const languages = [
      ...
      },
      {
        ...
        logo: "⚡",
        color: this.colors.yellow,
        features: ["V8 engine", "JIT compilation", "Dynamic typing"],
      },
      {
        x: 550,
        y: 250,
        ...
        logo: "🐍",
        color: this.colors.green,
        features: ["Interpreted", "Dynamic binding", "Rich ecosystem"],
      },
      {
        x: 750,
        y: 250,
        name: "Go",
        logo: "🚀",
        color: this.colors.sapphire,
        features: ["Garbage collected", "Concurrency", "Fast compilation"],
      },
    ];
    ...
    this.addText(
      ...
      450,
      505,
      "WASM Bytecode • Just-In-Time Compilation • Sandboxed Execution",
      {
        fontSize: "12px",
        textAnchor: "middle",
        fill: this.colors.subtext0,
  ...
  addTooltips(svg, tooltipData) {
    ...
    tooltipData.forEach((data) => {
      ...
      if (data.element) {
        ...
        element =
          svg.querySelector(`[cx="${data.element.x}"]`) ||
