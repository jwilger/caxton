...
class CaxtonSite {
  ...
  updateReleaseSection(data) {
    ...
    const statusBadge =
      data.version.includes("alpha") || data.version.includes("beta")
        ? "Pre-release"
        : "Latest";
  ...
  highlightRust(code) {
    const keywords =
      /\b(use|fn|impl|struct|enum|trait|async|await|match|if|else|for|while|loop|return|break|continue|let|const|mut|pub|mod|self|Self|super|crate|move|ref|where|type|unsafe|extern|static|as|in|from|into)\b/g;
    const types =
      /\b(String|Result|Ok|Err|Option|Some|None|Vec|HashMap|bool|u8|u16|u32|u64|i8|i16|i32|i64|f32|f64|usize|isize|char|str)\b/g;
    const strings = /("(?:[^"\\]|\\.)*")/g;
    const comments = /(\/\/[^\n]*)/g;
    const attributes = /(#\[[^\]]+\])/g;
  ...
  setupSearchUI() {
    ...
    const searchInputs = document.querySelectorAll(
      'input[type="search"], input[placeholder*="search"], #adr-search',
    );
  ...
  enhanceSearchInput(input) {
    ...
    if (!container.classList.contains("enhanced-search")) {
      ...
      instructions.textContent =
        "Type to search, use arrow keys to navigate results, Enter to jump to result, Escape to clear";
  ...
  bindSearchEvents() {
    ...
    input.addEventListener("input", (e) => {
      ...
      if (this.searchState.clearButton) {
        this.searchState.clearButton.style.display = hasValue
          ? "block"
          : "none";
      }
    });
    ...
    document.addEventListener("keydown", (e) => {
      ...
      if (
        (e.key === "F3" || ((e.ctrlKey || e.metaKey) && e.key === "g")) &&
  ...
  updateCurrentHighlight() {
    ...
    if (
      ...
      this.searchState.highlights[this.searchState.currentIndex].classList.add(
  ...
  updateResultCounter() {
    ...
    if (this.searchState.highlights.length === 0) {
      ...
      this.searchState.resultCounter.textContent = `${this.searchState.currentIndex + 1}/${this.searchState.highlights.length}`;
    }
  }
